 ...
 */
function loadConfig(configPath?: string): BemConfig {
  if (configPath) {
 ...
 */
function printResult(result: { errors: any[]; warnings: any[]; valid: boolean }): void {
  const RED = '\x1b[31m';
  ...
  for (const error of result.errors) {
    console.log(
      `${RED}${BOLD}✗ ${error.file}:${error.line}:${error.column}${RESET}`
    );
  ...
  for (const warning of result.warnings) {
    console.log(
      `${YELLOW}${BOLD}⚠ ${warning.file}:${warning.line}:${warning.column}${RESET}`
    );
  ...
  console.log('');
  console.log(
    `${result.errors.length > 0 ? RED : YELLOW}${BOLD}` +
